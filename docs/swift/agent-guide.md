---
description: "The Cog handbook on one page for a coding model, with the recurring code shapes, the conventions, and the lint rules that enforce them."
---
...
[handbook](./handbook/index.md) chapter is the full version, and the
[core design](./design/exploration.md) records why.

## Point your agent here

Agents do not read a dependency's docs unprompted. Give yours one of these:

- **Two lines in `AGENTS.md`, `CLAUDE.md`, `.github/copilot-instructions.md`,
  or a Cursor rule.** The `.md` URL is this page as plain Markdown.
  ...
  A Cursor rule can scope itself to the state layer, since every state file
  follows the family naming:

  ```markdown [.cursor/rules/cog.mdc]
  ---
  description: Cog state-management conventions
  globs: ["**/*State+*.swift", "**/*App.swift"]
  alwaysApply: false
  ---
...
- **The skill.** `npx skills add skeswa/cog` installs this page and the
  handbook chapters as an agent skill for Claude Code, Codex, Cursor, Copilot,
  Gemini CLI, and others. In Claude Code,
  `/plugin marketplace add skeswa/cog` then `/plugin install cog@cog` does the
  same as a plugin.

- **The whole site.** `https://skeswa.github.io/cog/llms.txt` indexes every
  page's Markdown twin, including the API reference and the lint rule
  articles that `coglint` diagnostics link to. Agents using the Context7 MCP
  server find the handbook under the `skeswa/cog` library.
...
::: tip Isolation
Every declaration and op below is MainActor-isolated. The example apps get
...
with `@MainActor`, as the [getting-started](./getting-started.md) tutorial
does.
:::
...
[structure](./handbook/app-structure.md),
[declaring](./handbook/declaring-state.md),
[reading](./handbook/reading-state.md),
[writing](./handbook/writing-state.md),
[SwiftUI](./handbook/swiftui.md),
[side effects](./handbook/side-effects.md),
[navigation](./handbook/navigation.md), and
[testing](./handbook/testing.md). The
