...
ordinary methods in extensions of `CogOperating` (§3.2), so `Cogs` and a
...
- `#function` names the turn without extra code. An op is just a normal
  method that calls `commit`, written in an extension of `CogOperating` — the
...
| Op, transaction, or turn?         | One named `commit`; ops are ordinary methods in `CogOperating` extensions, so `Cogs` and a mechanism's controller share every op (§3.2).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
2. **How much `Op` support ships in v1:** plain `CogOperating` extension
...
26. **Mechanisms:** settled on August 14, 2026. Side effects bundle into
    ...
    state-gated `whenever` scopes, and ops moved to `CogOperating` extensions.
