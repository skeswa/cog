...
```text
swift/Benchmarks/Macro/Storefront/
  README.md      this file
  Workload/      cog-storefront               the shared workload + the Cog port
  Runtimes/      cog-storefront-runtimes      the two plain-@Observable ports
  StateGraph/    cog-storefront-state-graph   the swift-state-graph port
  Apps/
    Cog/         the SwiftUI benchmark application driving the Cog port
```

`Workload/`, `Runtimes/`, and `StateGraph/` are three **separate SwiftPM
packages** that happen to sit in a subdirectory of the `cog-benchmarks` package
directory. That nesting is a filesystem fact and not a dependency fact, and it
must never be read as an invitation to merge them into `cog-benchmarks`: doing so
would break the iOS applications, which cannot resolve that package without also
resolving the ordo-one benchmark harness, its malloc interposer, and
swift-state-graph. Each package's own `README.md` states the argument in full;
none of them may be collapsed into the harness package.

Nothing under `Macro/` is a benchmark target. The executable benchmark targets
live next door in `swift/Benchmarks/Benchmarks/CogGraph/`, and the doubled name
there is upstream's hard discovery rule rather than a choice —
`swift/Benchmarks/Package.swift` records it.
...
| `observation-raw`  | `cog-storefront-runtimes`    | **The hardware floor.** Plain `@Observable` with no cache, no memo, no dirty bit, no dependency edge. Every derived value recomputed on every read.                           |
| `observation-memo` | `cog-storefront-runtimes`    | **The realistic competitor.** Honest hand-written memoization over the same primitives: seven caches and one file that says by hand what each write clears.                   |
...
The suite lives in `swift/Benchmarks/Tests/StorefrontAgreementTests`, because
nowhere else can hold it: `cog-storefront` cannot see the ports, and
`cog-storefront-runtimes` and `cog-storefront-state-graph` deliberately cannot
see each other. `cog-benchmarks` already depends on all four, so it is the one
place the four coexist without weakening that separation.
...
mise run test:storefront              # the workload package: census, shapes, Cog port
...
- [`Workload/README.md`](./Workload/README.md) — the shared workload, the two
  targets, the three profiles, the determinism rules, and the eleven-phase trace.
- [`Runtimes/README.md`](./Runtimes/README.md) — the floor and the hand-memoized
  competitor, their declared semantics, and every disclosed judgement call.
- [`StateGraph/README.md`](./StateGraph/README.md) and its
  [`API-NOTES.md`](./StateGraph/API-NOTES.md) — the swift-state-graph port and
  every measured library behavior it rests on, each with a `file:line` citation.
- [`swift/Benchmarks/README.md`](../../README.md) — the harness, the six
