...
the sibling `Workload` and `StateGraph` directories — that separation is the
whole design of this directory.
...
First, CogLint runs `swift/Benchmarks/Macro/Storefront/Workload/Sources` under
...
swift-state-graph port is in `swift/Benchmarks/Macro/Storefront/StateGraph` so
...
mistake [`Workload/README.md`](../Workload/README.md) documents next door about
...
Merging would break the iOS applications. `cog-benchmarks` depends on the
ordo-one benchmark harness, its malloc interposer, and swift-state-graph, and
SwiftPM hands those to everyone who resolves it — so an `@Observable` comparison
application under `Apps/` that linked a port from a merged package would resolve
all three, which is exactly the outcome the whole arrangement exists to prevent.
Neither may these ports be reached by a `cog-benchmarks` target's `path:`
pointing into `Macro/`; `cog-benchmarks` consumes them the same way any other
package would, through a path **dependency**, and that is the property to
preserve.
...
`swift/Benchmarks/Macro/Storefront/Workload/Package.swift` and must stay
...
import one another, so `StorefrontBuildShapeTests` in `StorefrontWorkloadTests`
reads all three manifests as text and fails when the copies drift apart.
...
package's and the state-graph port's.
