...
A **separate SwiftPM package**, and — like `swift/Benchmarks`, `swift/Lint`, and
the sibling `Workload` and `Runtimes` directories — that separation is the whole
...
`swift/Benchmarks/Macro/Storefront/Runtimes`, an `@Observable` comparison
...
[`Workload/README.md`](../Workload/README.md) documents next door about the
...
## Why nesting inside `swift/Benchmarks/` changes nothing

This directory lives inside the `cog-benchmarks` package directory. That is a
**filesystem fact, not a dependency fact**, and it must never be read as an
invitation to merge the two.

Merging would break the iOS applications. A comparison application under `Apps/`
that linked this port from a merged package would resolve the ordo-one benchmark
harness and its malloc interposer along with swift-state-graph, because SwiftPM
hands a package's dependencies to everyone who resolves it. This package is the
one place in the tree where swift-state-graph is allowed to reach an application
at all, and it reaches exactly one: the state-graph comparison app. Nor may a
`cog-benchmarks` target reach in here with a `path:`; it consumes this port
through a path **dependency**, the same way any other package would, and that is
...
The pin is `exact` rather than a range, matching `swift/Benchmarks/Package.swift`:
...
`swift/Benchmarks/Macro/Storefront/Workload/Package.swift` and must stay byte-identical to it. A comparison
...
`StorefrontWorkloadTests` gains an assertion that the three copies agree as the
ports land.
...
`swift/Benchmarks/Macro/Storefront/Workload/Sources/CogStorefront/StorefrontState.swift`,
...
package's and the two plain-Swift ports'.
