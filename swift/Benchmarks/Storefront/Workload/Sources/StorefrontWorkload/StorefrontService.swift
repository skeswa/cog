...
public nonisolated struct StorefrontService: Sendable {
  ...
  ///
  /// `public` because a runtime port lives in its own module and still has to
  /// reach the ledger: it releases scheduled requests by name and reads the
  /// start counts a checkpoint asserts on. Exposing the script rather than
  /// mirroring its API on the service keeps one bookkeeping surface, which is
  /// the only way four runtimes can be held to the same request accounting.
  public let script: StorefrontScript
  ...
  /// Registers work a runtime selected synchronously, before its task starts.
  ///
  ...
  /// release work even when the runtime has created its task but that task has
  /// not yet reached ``StorefrontScript/begin(_:)``. Immediate services record
  /// through the same path so both modes exercise identical bookkeeping.
  ///
  /// `public` because every runtime port selects its asynchronous work from
  /// its own module and must record that selection through this exact call,
  /// synchronously and on the MainActor, before launching any task.
  ///
  /// - Parameter id: The semantic request the selector chose.
  public func schedule(_ id: StorefrontRequestID) {
    script.schedule(id)
  }
...
public actor StorefrontScript {
  ...
  ///
  /// This ledger is lock-backed rather than actor-isolated because a runtime's
  /// asynchronous selector must register its request synchronously on the
  /// MainActor, before it launches the task that will serve the request and
  /// without an actor hop in between. The actor consumes the matching entry as
  /// that task begins.
  ...
  /// Records one synchronously selected request before its task launches.
  ///
  /// Nonisolated so a runtime's asynchronous selector can make the request
  /// visible without an actor hop. ``begin(_:)`` consumes exactly one matching
  /// entry later.
  ///
  /// - Parameter id: The semantic request the selector chose.
