...
The relationship runs one way only. This package depends on the root and the
independent Storefront packages by path; nothing in those manifests references
the runner. `M5-05a` proved the root half by describing both manifests. The
root graph contains only Cog itself, while the runner graph visibly owns the
harness and comparison dependencies:

```console
$ swift package show-dependencies --format json          # root
cog

$ swift package --package-path swift/Benchmarks/Runner show-dependencies --format json
benchmarks
  cog
  workload
  cogruntime
...
The Storefront workload, Cog runtime, Observation runtimes, and StateGraph
runtime are sibling package dependencies under [`../Storefront`](../Storefront/).
They remain independently usable by the Xcode applications and correctness
suites without resolving the benchmark harness or malloc interposer.
...
Three executable targets make the suite boundaries explicit:

- `CogCore` carries Cog-specific allocation, graph-shape, layout, boundary, and
  memory cuts.
- `RuntimeComparison` carries the shared synthetic shapes across Cog, raw
  Observation, and StateGraph.
- `Storefront` carries the application-shaped cuts across all four Storefront
  implementations.

The agreement suite is a correctness concern rather than a measurement target,
so it lives beside the application-shaped suite in
[`../Storefront/Verification`](../Storefront/Verification/). Baseline tolerances
pin counting metrics against drift. Committed static files add the portable CI
...
the package that declares them. Here that means
`swift/Benchmarks/Runner/Benchmarks/<Name>/`; the inner directory is upstream's
discovery rule.
...
`swift/Benchmarks/Runner/Package.resolved` is checked in: a measurement tool whose
...
The portable gate uses `swift package benchmark thresholds check` and the 17
files under `Thresholds/`: five exact CogCore allocation/ARC references and
twelve RuntimeComparison PERF-10 workload pairs. The wrapper fails before measuring if even one file is
...
live in **`swift/Benchmarks/Storefront/Workload`**, a package of its own, because the SwiftUI
benchmark application in `swift/Benchmarks/Storefront/Apps/Cog` drives the same workload
...
[`swift/Benchmarks/Storefront/Workload/README.md`](../Storefront/Workload/README.md) first.
...
`Benchmarks/Storefront/StorefrontRuntimeBenchmarks.swift`.
...
comparison between four runtimes mean anything at all. It lives in the sibling
test-only [`Storefront/Verification`](../Storefront/Verification/) package and
runs through `mise run test:storefront-agreement`.
...
their answers value for value. The runtime packages cannot see one another,
deliberately, since target and package separation make it a compile error for
one port to reach into another's cache. Verification is the sole correctness
package where all four coexist; this runner resolves them separately for
measurement only.
