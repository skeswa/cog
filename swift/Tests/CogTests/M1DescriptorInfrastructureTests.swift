...
@MainActor
@Test func `M1DescriptorInfrastructure gives every declaration its own identity`() {
...
@MainActor
@Test func `M1DescriptorInfrastructure keeps a declaration's identity stable`() {
...
@MainActor
@Test func `M1DescriptorInfrastructure labels a named declaration with its name`() {
...
@MainActor
@Test func `M1DescriptorInfrastructure labels an unnamed declaration with its file and line`() {
...
@MainActor
@Test func `M1DescriptorInfrastructure prints labels without knowing the value type`() {
...
@MainActor
@Test func `M1DescriptorInfrastructure keeps the starting value on the declaration`() {
