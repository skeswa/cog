...
@MainActor
@Test func `M1ManualCogBox puts the key on the value reference`() {
  let weather = ManualCogBox<Int, Int>(0)
...
@MainActor
@Test func `M1ManualCogBox gives every key its own state`() {
...
@MainActor
@Test func `M1ManualCogBox creates a key's state only when it is used`() {
  let cogs = Cogtext.forTesting()
...
@MainActor
@Test func `M1ManualCogBox keeps a state's key for diagnostics`() {
  let cogs = Cogtext.forTesting()
...
@MainActor
@Test func `M1ManualCogBox builds a value reference out of two inert words of storage`() {
