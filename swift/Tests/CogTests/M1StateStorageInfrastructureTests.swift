...
@MainActor
@Test func `M1StateStorage creates nothing until a declaration is used`() {
...
@MainActor
@Test func `M1StateStorage reuses one state for one declaration`() {
  let cogs = Cogtext.forTesting()
  let retryLimit = ManualCog<Int>(3)
...
@MainActor
@Test func `M1StateStorage resolves a copied value reference to the same state`() {
...
@MainActor
@Test func `M1StateStorage gives every key of a declaration its own state`() {
...
@MainActor
@Test func `M1StateStorage keeps a keyless declaration separate from its keys`() {
  let cogs = Cogtext.forTesting()
...
@MainActor
@Test func `M1StateStorage tells identical declarations apart`() {
...
@MainActor
@Test func `M1StateStorage gives every context its own states`() {
...
@MainActor
@Test func `M1StateStorage starts a state at its declaration's starting value`() {
  let cogs = Cogtext.forTesting()
  let retryLimit = ManualCog<Int>(3)
...
@MainActor
@Test func `M1StateStorage keeps a state's label and key for diagnostics`() {
  let cogs = Cogtext.forTesting()
