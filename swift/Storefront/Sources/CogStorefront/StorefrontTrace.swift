...
extension StorefrontSessionDriver {
  ...
  /// reaction ran once, found nothing on screen, and started exactly three
  /// requests. A graph that eagerly demanded per-row work before it had rows
  /// would fail here rather than merely being slower.
  ///
  ...
  public func runRootDataPhase() async throws {
    ...
    // Two runs, and both are real: `initial: .run` fires the watch during
    // bootstrap against the resting `nil`, and the accepted account fires it
    // again. A watch that only fired on change would leave the signed-out world
    // unwritten.
    check(phase: phase, "account runs", expected: 2, actual: sink.accountRuns)
  ...
  public func runFilterPhase() async throws {
    ...
    cogs.applyBrowseFilters(category: category, sortMode: .priceAscending, inStockOnly: true)
    ...
    cogs.selectSortMode(.priceDescending)
    ...
    // claim: an equal write settles and renders nothing.
    before = sink.browseRuns
    cogs.selectSortMode(.priceDescending)
    check(
      phase: phase,
    ...
    cogs.selectSortMode(.relevance)
    world.sortMode = .relevance
    cogs.setInStockOnly(false)
  ...
  public func runCartPhase() async throws {
    ...
    for (position, id) in cartIDs.enumerated() {
      cogs.addToCart(id, quantity: position + 1)
  ...
  public func runDetailPhase() async throws {
    ...
    nextViewRank += 1
    cogs.openProduct(id, rank: nextViewRank)
  ...
  public func runCheckoutPhase() async throws {
    ...
    for quantity in [4, 2, 5] {
      cogs.setCartQuantity(quantity, for: firstLine)
      world.cartQuantities[firstLine] = quantity
    }

    cogs.applyCoupon(StorefrontFixtures.sessionCoupon)
    world.coupon = StorefrontFixtures.sessionCoupon

    cogs.selectShippingMethod(.express)
    ...
    check(
      ...
      actual: describeStrings(currentPromotionPlan().appliedIDs)
    )
  }
  ...
  /// test clock through lifetime grace to prove release.
  ///
  ...
  public func runTeardownPhase() async throws {
    ...
    cogs.scrollRows(to: RowWindow(offset: 0, length: 0))
    ...
    // never be satisfied.
    cogs.scrollRows(to: RowWindow(offset: 0, length: profile.viewportRowCount))
  ...
  ///
  /// A `peek` rather than a tracked read, because this is a checkpoint reading
  /// a settled value rather than a screen observing one.
  func currentPrice(of id: ProductID) -> Int {
    cogs.peek(storefrontEffectivePriceCogs[id])
  }

  /// The promotion plan the graph currently reports.
  func currentPromotionPlan() -> PromotionPlan {
  ...
  func describeOutcome(_ outcome: CogRefresh<[ProductID]>.Outcome) -> String {
    switch outcome {
