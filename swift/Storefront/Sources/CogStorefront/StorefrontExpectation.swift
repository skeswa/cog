...
public nonisolated struct StorefrontWorld: Sendable {
  ...
  ///
  /// Mirrors ``storefrontFilterEligibilityCogs`` exactly, including its choice
  /// to consult the catalog's stock rather than live inventory.
  ///
  ...
  ///
  /// Mirrors ``storefrontSectionsCog``: a cross-category best-match band of at
  /// most twelve, then the remainder grouped by category in first-seen order.
