...
Open `TodoMVC.xcodeproj`, or use the guarded repository commands:
...
- `TodoState.swift` keeps ordered membership in one manual cog and each row's
...
- `TodoMechanism` restores persisted JSON during assembly and owns the one
  persistence reaction. `UserDefaults` is durable storage, not a second live
  source of truth.
...
- Tests create isolated runtimes through `CogTesting` and prove both behavior
  and Observation invalidation boundaries.
