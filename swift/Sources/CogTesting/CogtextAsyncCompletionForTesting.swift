...
extension Cogtext {
  ...
  /// cancelled, state-released, or dependency-invalidated completion that Cog
  /// drops while this context remains alive. It does not alter settlement or
  /// install a graph consumer.
  ///
  ...
  ///
  /// Keep the context alive until the acknowledgement fires. Work tasks capture
  /// their context weakly, and the callback is stored on that context, so a
  /// result returning after context teardown has neither a graph decision nor a
  /// surviving callback to acknowledge. The seam deliberately does not extend
  /// context lifetime; a test that exercises state release should retain the
  /// context and await this signal before dropping it.
  ///
