/// One bundled family of side effects, registered at bootstrap.
///
...
/// retains the exact mechanism values supplied at bootstrap alongside their
...
/// ```
///
/// Mechanisms are specified only at `Cogs.bootstrapApp(mechanisms:)` — or the
/// `CogTesting` factory's matching parameter — and each `operate` runs
/// synchronously in array order before bootstrap returns. Declaring a
/// mechanism does nothing by itself; a mechanism left off the bootstrap list
/// never runs (§6.3).
@MainActor
public protocol Mechanism {
  ...
  /// Bootstrap rejects two mechanisms that share a name, in debug and release
