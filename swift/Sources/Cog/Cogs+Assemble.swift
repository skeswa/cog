...
extension Cogs {
  /// The app's context, once assembly has installed it.
  ///
  ...
  /// after assembly, independently of scene recreation.
  private static var installedContext: Cogs?
  ...
  /// assembly creates no individual Cog state; the graph remains lazy until
  ...
  ///   this exact reference at the app root rather than assembling again.
  @discardableResult
  public static func assemble(mechanisms: [any Mechanism] = []) -> Cogs {
    let cogs = Cogs(
      clock: ContinuousClock(),
      defaultWhileObservedGrace: .seconds(30)
    )
    installAsContext(cogs)
    cogs.operateMechanisms(mechanisms)
    return cogs
  }

  /// The app's context, or `nil` when nothing has assembled one.
  ///
  /// Package access is only for the `CogTesting` assembly fixture to verify
  /// installation identity without making production code depend on global
  /// lookup.
  package static var installedCogs: Cogs? {
    installedContext
  }
  ...
  private static func installAsContext(_ cogs: Cogs) {
    guard installedContext == nil else {
      fatalError(
        """
        Cog is already assembled. `Cogs.assemble(mechanisms:)` \
        ...
        every scene rather than assembling again. Tests and previews are \
        ...
        `Cogs.withAssembledCogs { }` — which is not re-entrant, so do \
        not nest it.
        """
      )
    }
    installedContext = cogs
  }

  /// Forgets the app's context, leaving the process as it was before
  /// assembly.
  ///
  /// Used by `CogTesting` to restore the process after a scoped assembly test.
  ...
  package static func uninstallCogs() {
